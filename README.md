...
Allow me to demonstrate Morpheus in action in this simple example, in which we will be modelling a simple chat application. Such an application inherently features a lot of dynamics, while the most obvious one is changing a contact's status. So let’s begin with modelling the contact and its status.
