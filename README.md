...
Morpheus is a Scala extension bringing metamorphosing capabilities into Scala objects. As the name suggests, this component allows for an object to morph (or mutate) into various forms virtually anytime either as a result of external or internal events. However, the forms that the object can adopt are not arbitrary. Instead, all possible forms of the object are determined by a finite automaton model represented by so-called morph type. The morph type declares two fundamentals: firstly the building blocks called fragments and secondly all possible combinations of fragments representing the object forms called alternatives. Morph types are expressed as Scala types and are checked in compile time so that the runtime part of Morpheus can be sure that all forms of an object under construction are valid and assembled correctly from the fragments. In this way Morpheus introduces a controlled and type-safe dynamics into strictly typed Scala.

Let me show Morpheus in action on a simple example, in which we will be modelling a simple chat application. Such an application inherently features a lot of dynamics, while the most obvious one is changing a contact's status. So let’s begin with modelling the contact and its status.

The contact is undoubtedly the fundamental entity in the application. It represents a remote user registered in the profile of the local user of the chat application. In contrast to the traditional OOP approach, in which an entity is represented by means of a class, here, we use traits for modelling entities. By the way, classes are second-class citizens in Morpheus and are only used as auxiliary types.
